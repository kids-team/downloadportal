...
const LanguageSelection = ({languages}: Props) => {
	...
	return (
		...
		<li onClick={() => {setOpen(!open)}} className={'menu__item language-menu menu__item--last ' + (open?'menu__item--open':'')}>
			<a href="#/"><span><img className="menu__icon lg:show lg:mr-0" height="12px" src={getFlag(getFlagCode(state.lang))} /><span className="lg:hidden language__name">{countryNames[state.lang]}</span></span>
