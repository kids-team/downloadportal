        ...
        <>
            <div className={'search ' + className + (searchOpen ? ' search--open' : '')}>
                <button
                    ...
                    type="button"
                    className={'search__button ' + (searchOpen ? 'active' : '')}
                >
                ...
                {searchOpen && (
                    <div className="search__form">
                        ...
                        <div
                            className={'search__results ' + (searchOpen ? 'search__results--visible' : '')}
                            onClick={event => {
                            ...
                            <ul className="list list--padded">
                                {response.more && (
                                    <li className="list__item">
                                        <FormattedMessage defaultMessage="Hit Enter for more results" id="hitEnter" />
                                    </li>
                                )}
                                {response.result?.map((item: Result, index) => {
                                    return (
                                        <a
                                            key={index}
                                            onClick={() => {
                                                openSearchResult(item.id);
                                            }}
                                            className={
                                                'list__item ' +
                                                (selectedSearchItem === index ? 'list__item--active' : '')
                                            }
                                        >
                                            <span>
                                                {item.title}
                                                <br />
                                                <sub className="list__subtitle">{item.abstract}</sub>
                                            </span>
                                        </a>
                                    );
                                })}
